def get_store_price(self, store):
    ...
    elif self.market.my_reduction.filter(store=store).exclude(reduction_percentual=100):							
        self.price = self.price * self.market.my_reduction.get(store=store).reduction_percentual / 100										
    else:					
