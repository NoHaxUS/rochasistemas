...
def consuming_championship_api():
	...
	for actual_page in range(1, total_pages):
		request = requests.get("https://soccer.sportmonks.com/api/v2.0/leagues/?api_token="+TOKEN+"&include=country&tz=America/Santarem&page="+str(actual_page+1) )
...
def consuming_game_cotation_api():
	...
	for actual_page in range(1, total_pages):
		next = 'https://soccer.sportmonks.com/api/v2.0/fixtures/between/'+first_date+'/'+second_date+'?page='+str(actual_page+1)+'&api_token='+TOKEN+'&include=localTeam,visitorTeam,odds&tz=America/Santarem'
